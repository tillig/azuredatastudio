...
export class ConnectionManagementService extends Disposable implements IConnectionManagementService {
	...
	public showConnectionDialog(params?: INewConnectionParams, options?: IConnectionCompletionOptions, model?: IConnectionProfile, connectionResult?: IConnectionResult): Promise<void> {
		...
		return this._connectionDialogService.showDialog(this, params, model, connectionResult, options).then(undefined, dialogError => {
	...
	private async connectWithOptions(connection: IConnectionProfile, uri: string, options?: IConnectionCompletionOptions, callbacks?: IConnectionCallbacks):
		Promise<IConnectionResult> {
	...
	public editGroup(group: ConnectionProfileGroup): Promise<void> {
		return this._connectionStore.editGroup(group).then(() => {
			this._onAddConnectionProfile.fire(undefined);
		});
	}
			...
			this.doDisconnect(uri, connection).then((result) => {
				if (result) {
					...
					this._connectionStore.deleteConnectionFromConfiguration(connection).then(() => {
						...
						Promise.resolve(true); // TODO @anthonydresser this doesn't do anything
					}).catch(err => {
						// Reject promise if error occurred writing to settings
						Promise.reject(err); // TODO @anthonydresser this doesn't do anything
					});

				} else {
					// If connection fails to disconnect, resolve promise with false
					Promise.resolve(false); // TODO @anthonydresser this doesn't do anything
				}
			});
		} else {
	...
	public deleteConnectionGroup(group: ConnectionProfileGroup): Promise<boolean> {
		...
		Promise.all(disconnected).then(() => {
			...
			this._connectionStore.deleteGroupFromConfiguration(group).then(() => {
				...
				throw false; // TODO @anthonydresser this doesn't do anything
			});
		}).catch(err => {
			// If disconnecting all connected profiles fails, resolve promise with false
			return Promise.resolve(false); // TODO @anthonydresser this doesn't do anything
		});
		return Promise.resolve(undefined);
	}
